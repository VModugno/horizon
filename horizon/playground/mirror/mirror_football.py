...
for contact in contacts:
    ...
    if swing_nodes[contact]:
        ...
        # velocity
        kick_vel_constr[contact] = prb.createConstraint(f"{contact}_kick_vel", ee_v[0], nodes=[28, 29])
        kick_vel_constr[contact].setBounds(kick_vel, kick_vel)

        kick_pos_y[contact] = prb.createConstraint(f"{contact}_kick_pos_y", ee_v[1], nodes=[28, 29])


        kick_pos[contact] = prb.createConstraint(f"{contact}_kick_pos", ee_pos[0], nodes=[29])
        kick_pos[contact].setBounds(contact_pos[contact][0] + kick_lenght, contact_pos[contact][0] + kick_lenght)


        # print(swing_nodes[contact][0] + int(len(swing_nodes[contact])/2))
        # traj_new = [elem for elem in swing_nodes[contact] if elem not in [35, 36, 37, 38]]
        # clearance
        contact_pos[contact] = FK(q=model.q0)['ee_pos']
        ...
        # clea[contact] = prb.createResidual(f"{contact}_clea", 1e5 * (ee_pos[2] - z_des[contact]), nodes=swing_nodes[contact])
...
# ====================store solutions ============================
name_stored = f'mat_files/mirror_football.mat'
