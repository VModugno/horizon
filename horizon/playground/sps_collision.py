...
p_tgt_A = p_start_A + np.array([0.3, -0.1, 0.0])
p_tgt_B = p_start_B + np.array([-0.1, +0.1, 0.0])
...
collision_barrier = cs.if_else(collision_sx > 0, 0, collision_sx)
...
##### Cost function #####
# penalize input
prb.createIntermediateResidual('min_u', np.sqrt(1) * q_ddot)
...
# prb.createIntermediateResidual('min_qdot_rotary', np.sqrt(1e4) * q_dot)
